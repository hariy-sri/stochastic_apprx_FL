...
def process_client_epoch(client_id, X, Y, theta, base_lr, batch_counter, N):
    ...
    # Pre-compute learning rate
    is_client_zero = client_id == 0
    lr_denominator = (batch_counter + 10) ** (3/4) if is_client_zero else batch_counter
...
for N in N_values:
    ...
    for r in range(rounds):
        for client_id in range(num_clients):
            ...
            base_lr = 1.0 if client_id == 0 else 0.01
    ...
    gradient_means[N] = {"client0": gradient_client0}
...
# Generate plots
plot_theta_convergence(global_theta_sgd_trajectories, rounds, reference=clients_data[0][0], 
                      output_path=f"{save_dir}/1_vector_sgd_convergence_components.png")
plot_local_gradients_multiple_N(local_gradient_sgd_trajectories_N, 
                              output_path=f"{save_dir}/1_local_gradients_multiple_N.png")
plot_local_thetas_multiple_N(local_thetas_sgd_trajectories_N, 
                            output_path=f"{save_dir}/1_local_thetas_multiple_N.png")
plot_gradients_multiple_N(local_gradient_sgd_trajectories_N, 
                         output_path=f"{save_dir}/1_gradients_multiple_N.png")
plot_thetas_multiple_N(local_thetas_sgd_trajectories_N, 
                      output_path=f"{save_dir}/1_thetas_multiple_N.png")